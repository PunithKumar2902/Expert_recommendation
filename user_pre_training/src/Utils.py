...
def type_loss(prediction, label, event_time, test_label, opt):
    ...
    multi_hots = torch.zeros(label.size(0), C.TAG_NUMBER, device='cuda:0', dtype=torch.float32)
    ...
    multi_hots = multi_hots * (1 - opt.smooth) + (1 - multi_hots) * opt.smooth / C.TAG_NUMBER
