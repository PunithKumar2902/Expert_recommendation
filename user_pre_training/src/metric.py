...
def pre_rec_top(pre, rec, map_, ndcg, prediction, label, event_type):
    ...
    target_ = torch.ones(event_type.size()[0], C.TAG_NUMBER, dtype=torch.double)
    event_type1 = event_type.clone().detach().to("cpu")
    prediction1 = prediction.clone().detach().to("cpu")

    for i, e in enumerate(event_type1):
        try :
            e = e[e!=0]-1
            target_[i][e] = 0
        ...
        # print(e)
    # print("prediction dev :",prediction1.device)
    # print("target dev :",target_.device)

    prediction1 = prediction1 * target_

    label1 = label.clone().detach().to("cpu")
